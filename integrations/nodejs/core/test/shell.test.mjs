...
{
  const shell = loadShell({ collectionUrl: './c', pageTitle: 'Acme API' });
  ...
  assert.equal(loadShell({ collectionUrl: './c' }).html('/x/').body.match(/<title>(.*)<\/title>/)[1], 'API Documentation');
  assert.ok(loadShell({ collectionUrl: './c' }).html('/a/b/c/').body.includes('data-base="/a/b/c/"'), 'any depth');
}

// --- the CSP promise: nothing inline
{
  const html = loadShell({ collectionUrl: './c' }).html('/docs/').body;
...
{
  const shell = loadShell({ collectionUrl: './c' });
  const res = shell.js({});
  ...
  assert.equal(loadShell({ collectionUrl: './c', pageTitle: 'Other' }).js({}).headers.ETag, res.headers.ETag,
...
{
  assert.equal(loadShell({ collectionUrl: './c' }).html('/docs/').headers['Cache-Control'], 'no-cache');
}
